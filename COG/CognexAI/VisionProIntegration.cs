    ...
    internal class VisionProIntegration : INotifyPropertyChanged
    {
     ...
     //       RunVisionProToolsAndSetUpFixture(cogImage, sample);
        ...
        private List<ICogRecord> RunDeepLearningToolsAndGetRecords(IImage image, ISample sample)
        {
            ...
            {
                GetToolsName.Add(toolitem.Type.ToString() , toolitem.Name);
            ...
            foreach (var ToolType in GetToolsName)
            {
                switch (ToolType.Key)
                {
                    case "Blue":
                        ...
                        var MarkingBlue = samples.Markings[ToolType.Value] as ViDi2.IBlueMarking;
                        toolRecords.Add(CreateToolRecordFromMarking(MarkingBlue, image, ToolType.Value));
                    ...
                    case "카본":
                        ...
                        CogGraphicCollection graphicCollectionRed = new CogGraphicCollection();
                            ...
                            var MarkingRed = samples.Markings[ToolType.Value] as ViDi2.IRedMarking;
                 //           toolRecords.Add(CreateToolRecordFromMarking(MarkingRed, image, ToolType.Value));
