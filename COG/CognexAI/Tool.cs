...
namespace CognexAI
{
    public class CognexAIControl 
    {
        ...
        {
            var workspace = Control.Workspaces.Add(System.IO.Path.GetFileNameWithoutExtension(filePath), filePath);

            if (IsRuntimeWorkspaceValid(workspace))
            {
                Workspace = workspace;
                WorkspaceName = System.IO.Path.GetFileName(filePath);
            }
            else
            {
                Control.Workspaces.Remove(workspace.UniqueName);
            }
        }
        ...
        public List<ViDi2.ITool> GetTools()
        {
            List<ViDi2.ITool> returnValue = new List<ViDi2.ITool>();
            ...
            foreach (var items in GetToolsName)
            {
                returnValue.Add(Stream.Tools[items.Key]);        
            }
              return returnValue;
        }
    ...
    public class Predictor : INotifyPropertyChanged
    {
        private IControl _control;
        /// <summary>
        /// Gets or sets the main control providing access to the library
        ...
        public ISample Run(IImage image)
        {
              return Stream.Process(image);
        }
        ...
        public ICogRecord RunTools(IImage image)
        {
            ...
            var toolRecords = RunDeepLearningToolsAndGetRecords(image);
        ...
        private List<ICogRecord> RunDeepLearningToolsAndGetRecords(IImage image)
        {
            ...
            foreach (var toolitem in Stream.Tools)
            {
                GetToolsName.Add(toolitem.Name ,toolitem.Type.ToString());
                GetToolName(GetToolsName, Stream.Tools[toolitem.Name].Children);
            }
        ...
        public SortedList<string, string> GetToolNames(IStream _Stream)
        {

            var Return = new SortedList<string, string>();

            SortedList<string, string> GetToolsName = new SortedList<string, string>();
            foreach (var toolitem in _Stream.Tools)
            {
                GetToolsName.Add(toolitem.Name, toolitem.Type.ToString());
                GetToolName(GetToolsName, _Stream.Tools[toolitem.Name].Children);
            }
        ...
        private void GetToolName(SortedList<string, string> ListToolsName, ViDi2.IToolList<ViDi2.Runtime.ITool> _Children)
        {
            ...
            try
            {
                foreach (var Tool in _Children)
                {
                   // ListToolsName.Add(Tool.Type.ToString(), Tool.Name.ToString());
                    ListToolsName.Add(Tool.Name.ToString() , Tool.Type.ToString());
